 ...
 * @param {number} width
 * @param {number} height
 * @param {any} returnRawValues
 */
function getCharacteristicsForArea(x, y, width, height, returnRawValues) {
    ...
    let { mean_luminance, luminance_variance, mean_acutance, acutance_variance } =
    ...
    if (mean_luminance < LUMINANCE_DARK_THRESHOLD)
        areaIsDark = true;
    else if (mean_luminance > LUMINANCE_BRIGHT_THRESHOLD)
        areaIsBright = true;

    if (mean_acutance > ACUTANCE_NOISY_THRESHOLD ||
        (mean_acutance * 4 > ACUTANCE_NOISY_THRESHOLD &&
            acutanceStd > ACUTANCE_STD_NOISY_THRESHOLD) ||
        luminanceStd > LUMINANCE_STD_NOISY_THRESHOLD ||
        (areaIsDark &&
            mean_luminance + luminanceStd > LUMINANCE_BRIGHT_THRESHOLD) ||
        (areaIsBright &&
            mean_luminance - luminanceStd < LUMINANCE_DARK_THRESHOLD))
        areaIsNoisy = true;

    if (returnRawValues)
        return [true, areaIsNoisy, areaIsDark, areaIsBright, mean_luminance, luminanceStd, mean_acutance, acutanceStd];
    else
