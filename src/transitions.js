...
const { settings: Settings, theming: Theming } = Me.imports;
 ...
 */
function update_corner_alpha(override_alpha) {
  ...
  if (!alpha) {
    if (Settings.get_hide_corners()) {
      ...
      alpha = this.status.is_transparent()
        ? Theming.get_unmaximized_opacity()
        : Theming.get_maximized_opacity();
    }
  }
...
function fade_out() {
  ...
  if (!Settings.get_hide_corners()) {
    const speed = St.get_slow_down_factor() * Settings.get_transition_speed();

    const unmaximized = Settings.get_unmaximized_opacity();
    const maximized = this.status.is_transparent() ? unmaximized : Settings.get_maximized_opacity();
...
function fade_in() {
  ...
  if (!Settings.get_hide_corners()) {
    ...
    const maximized = Settings.get_maximized_opacity();
    const unmaximized = !this.status.is_transparent()
      ? maximized
      : Settings.get_unmaximized_opacity();
