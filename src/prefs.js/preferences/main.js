...
const {
  gi: { GLib, GObject, Gdk, Gio, Gtk },
  preferences: { appChooser: AppChooser, appRow: AppRow, tweaks: Tweak }
} = imports;
...
function buildPrefsWidget() {
  ...
  main_notebook.connect(
    'switch-page',
    function(notebook, page, index) {
  ...
  /* Setup foreground tab */
  {
    ...
    let maximized_text_color_switch = builder.get_object(
    ...
    let maximized_text_color_btn = builder.get_object(
    ...
    let text_shadow_vertical_offset = builder.get_object(
      'text_shadow_vertical_offset'
    );
    settings.set_value(
    ...
    text_shadow_vertical_offset.set_value(
      settings.get_value(SETTINGS_TEXT_SHADOW_POSITION).deep_unpack()[
        VERTICAL_OFFSET
      ]
    );
    text_shadow_vertical_offset.connect(
      ...
      function(widget) {
        let position = settings
          .get_value(SETTINGS_TEXT_SHADOW_POSITION)
          .deep_unpack();
    ...
    let text_shadow_horizontal_offset = builder.get_object(
    ...
    let text_shadow_color = settings
      .get_value(SETTINGS_TEXT_SHADOW_COLOR)
      .deep_unpack();
    ...
    text_shadow_color_btn.connect(
      ...
      function(color_btn) {
        ...
        settings.set_value(
          SETTINGS_TEXT_SHADOW_COLOR,
          new GLib.Variant('(iiid)', rgba)
        );
      }.bind(this)
    );
    ...
    let icon_shadow_vertical_offset = builder.get_object(
    ...
    icon_shadow_vertical_offset.set_value(
      settings.get_value(SETTINGS_ICON_SHADOW_POSITION).deep_unpack()[
        VERTICAL_OFFSET
      ]
    );
    icon_shadow_vertical_offset.connect(
      ...
      function(widget) {
        let position = settings
          .get_value(SETTINGS_ICON_SHADOW_POSITION)
          .deep_unpack();
        position[VERTICAL_OFFSET] = widget.get_value_as_int();
        settings.set_value(
          SETTINGS_ICON_SHADOW_POSITION,
    ...
    let icon_shadow_horizontal_offset = builder.get_object(
      'icon_shadow_horizontal_offset'
    );
    ...
    let icon_shadow_color = settings
      .get_value(SETTINGS_ICON_SHADOW_COLOR)
      .deep_unpack();
  ...
  /* Setup Background Tab */
  {
    ...
    let background_color_revealer = builder.get_object(
  ...
  /* Setup App Settings Tab */
  {
    ...
    let cfg = function(app_name, app_id, path, extras = []) {
      ...
      let background_tweaks_switch = app_prefs_builder.get_object(
        'background_tweaks_switch'
      );
      let background_tweaks_revealer = app_prefs_builder.get_object(
        'background_tweaks_revealer'
      );
      background_tweaks_switch.set_active(
        app_settings.get_boolean(SETTINGS_ENABLE_BACKGROUND_TWEAKS)
      );
      background_tweaks_revealer.set_reveal_child(
        background_tweaks_switch.get_active()
      );
      background_tweaks_switch.connect(
        'state-set',
        function(widget, state) {
      ...
      /* Init value. */
      _maximum_scale.adjustment.set_value(
        app_settings.get_int(SETTINGS_MAXIMIZED_OPACITY)
      );
      /* Add formatting */
      ...
      if (response === Gtk.ResponseType.APPLY) {
        if (temp_app_settings.background_tweaks !== null)
          app_settings.set_value(
            SETTINGS_ENABLE_BACKGROUND_TWEAKS,
        ...
        for (let extra of extras) {
          ...
          let extra_obj = Convenience.getSchemaObj(
            'org.gnome.shell.extensions.dynamic-panel-transparency.appOverrides'
          );
          let extra_settings = new Gio.Settings({
    ...
    for (let override of window_overrides) {
      ...
      if (tweak) {
        ...
        if (!found) {
          let extra_wm_class =
            tweak.wm_class.length <= 1 ? [] : tweak.wm_class.slice(1);
          let row = new AppRow.CustomRow(
            tweak.name,
            tweak.wm_class[0],
            tweak_cfg,
            tweak_rmv,
    ...
    add.btn.connect(
      ...
      function() {
        ...
        let overrides = settings.get_strv('app-overrides');
        let a2 = new AppChooser.AppChooser(
          main_widget.get_toplevel(),
          overrides
        );
        a2.show_all();
          ...
          if (selected_app) {
            if (typeof selected_app === 'string') {
              ...
              if (tweak) {
                ...
                if (
                  typeof tweak.trigger !== 'undefined' &&
                  tweak.trigger !== null
                ) {
                  let triggers = settings.get_strv('trigger-windows');
                  for (let wm_class of tweak.wm_class) {
