...
function cleanup() {
  ...
  this.styles.forEach(style => Panel.actor.remove_style_class_name(style));

  this.background_styles.forEach(style => Panel.actor.remove_style_class_name(style));
...
function has_icon_shadow() {
  return (
    Panel.actor.has_style_class_name('dpt-panel-icon-shadow')
    || Panel.actor.has_style_class_name('dpt-panel-arrow-shadow')
  );
}
 ...
 */
function register_text_color(color, def_prefix) {
 ...
 */
function set_text_color(def_prefix) {
  let prefix;
 ...
 * @param {string} prefix - What stylesheet prefix to retrieve. '-' is the default.
 */
function remove_text_color(def_prefix) {
  let prefix;
...
function set_corner_color(def_color) {
...
function get_background_color() {
  const custom = Settings.get_panel_color({ app_info: true });

  if (custom.app_info !== null && Settings.check_overrides()) {
    if (
      Settings.window_settings_manager.enable_background_tweaks[custom.app_info]
      || Settings.app_settings_manager.enable_background_tweaks[custom.app_info]
    ) {
      return custom.value;
    }
    ...
    const original = Settings.get_panel_color({ app_settings: false });
    return original;
  }
...
function get_maximized_opacity() {
  const maximized_opacity = Settings.get_maximized_opacity({ app_info: true });
  ...
  if (maximized_opacity.app_info !== null && Settings.check_overrides()) {
    if (
      Settings.window_settings_manager.enable_background_tweaks[maximized_opacity.app_info]
      || Settings.app_settings_manager.enable_background_tweaks[maximized_opacity.app_info]
    ) {
 ...
 */
function average_color(source, def_width, def_height) {
  let r;
  let g;
  let b;
  let a;
...
function initialize_background_styles() {
  ...
  const tweaked_apps = Object.keys(Settings.app_settings_manager.enable_background_tweaks);
  const tweaked_windows = Object.keys(Settings.window_settings_manager.enable_background_tweaks);
  ...
  tweaked_apps.forEach((key) => {
    ...
    if (Settings.app_settings_manager.maximized_opacity[key]) {
      register_background_color(
        Util.tuple_to_native_color(Settings.app_settings_manager.panel_color[key]),
        prefix,
        'tweaks',
        Settings.window_settings_manager.maximized_opacity[key]
      );
    } else {
      register_background_color(
        Util.tuple_to_native_color(Settings.app_settings_manager.panel_color[key]),
  ...
  tweaked_windows.forEach((key) => {
    ...
    if (Settings.window_settings_manager.maximized_opacity[key]) {
      register_background_color(
        Util.tuple_to_native_color(Settings.window_settings_manager.panel_color[key]),
        prefix,
        'tweaks',
        Settings.window_settings_manager.maximized_opacity[key]
      );
...
function register_background_color(
  bg_color,
  def_prefix,
  def_tweak_name,
  ...
  const maximized_opacity = Util.clamp(
    (def_maximized_opacity || get_maximized_opacity()) / SCALE_FACTOR,
    0,
    1
  ).toFixed(2);
  const unmaximized_opacity = Util.clamp(
    (def_unmaximized_opacity || get_unmaximized_opacity()) / SCALE_FACTOR,
    0,
    1
  ).toFixed(2);
...
function set_unmaximized_background_color(def_prefix) {
  let prefix;
...
function set_maximized_background_color(def_prefix) {
  let prefix;
...
function remove_unmaximized_background_color(def_prefix) {
  let prefix;
...
function remove_maximized_background_color(def_prefix) {
  let prefix;
...
function remove_background_color(def_params) {
  ...
  const excluded_unmaximized_style = `${prefix === null ? null : `dpt-panel${prefix}`}unmaximized`;
  ...
  this.background_styles
    ...
    .forEach(style => Panel.actor.remove_style_class_name(style));
}
